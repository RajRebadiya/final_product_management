...
<body>
    ...
    @endif
    <form method="GET" id='search-form' action="{{ route('barcode1', ['id' => $product->p_name ?? null]) }}"
    ...
    <div class="barcode-container" id="barcode-container">
        <div class="barcode-item"
            style="width: 170px;height: 82px;box-sizing: border-box;padding: 12px;margin-top: -5px; ">

            <div>
                {!! DNS1D::getBarcodeHTML($product->p_name, 'C128') !!}
            </div>
            <p style="margin: 0px; font-size: 21px;">D.No - {{ $product->p_name }}</p>
        </div>
    </div>
